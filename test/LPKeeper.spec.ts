...
describe('LPKeeper', () => {
  ...
  describe('#add', () => {
    it('fails if caller is not allowed to add', async () => {
      await expect(lpKeeper.add(token1.address, other.address, 1)).to.be.reverted
    })
    ...
    it('adds to user balance', async () => {
      ...
      expect(await lpKeeper.userBalance(token1.address, wallet.address)).to.eq(1)
    })
  ...
  describe('#subtract', () => {
    it('fails if caller is not allowed to subtract', async () => {
      await lpKeeper.setCanAdd(wallet.address, true)
      await lpKeeper.add(token1.address, other.address, 1)
      await expect(lpKeeper.subtract(token1.address, other.address, 1)).to.be.reverted
    })
