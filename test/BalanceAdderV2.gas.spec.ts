...
describe("BalanceAdderV2Old", () => {
    ...
    async function deploy() {
        ...
        impactEB = (await impactEBFactory.deploy(
            wallet.address,
            nebula.address,
        ...
        const farmEBFactory = await ethers.getContractFactory(
            "MockTimeFarmCurved"
        )
        ...
        sharesLP1 = (await sharesLPFactory.deploy(
            lpKeeper.address,
            0
        )) as SharesLP

        const farmLPFactory = await ethers.getContractFactory(
            "MockTimeFarmLinear"
        )
        ...
        sharesLP2 = (await sharesLPFactory.deploy(
            lpKeeper.address,
            1
        )) as SharesLP
    ...
    async function processBalancesV2OldCost(): Promise<BigNumber> {
        ...
        let zeroFarm = async () => (await balanceAdderV2Old.currentFarm()).eq(BigNumber.from(0))
        let zeroUser = async () => (await balanceAdderV2Old.currentUser()).eq(BigNumber.from(0))
        while (true) {
            gas = gas.add(await gasCost(balanceAdderV2Old.processBalances(50)) ?? 0)
            if ((await zeroFarm()) && (await zeroUser())) { break }
        }
        return gas
    }
    async function processBalancesV2Cost(): Promise<BigNumber> {
        ...
        let zeroFarm = async () => (await balanceAdderV2.currentFarm()).eq(BigNumber.from(0))
        let zeroUser = async () => (await balanceAdderV2.currentUser()).eq(BigNumber.from(0))
        while (true) {
            gas = gas.add(await gasCost(balanceAdderV2.processBalances(50)) ?? 0)
            if ((await zeroFarm()) && (await zeroUser())) { break }
        }
