...
describe('LPKeeperV2', () => {
  ...
  it('starting state after deployment', async () => {
    expect(await lpKeeper['isKnownToken(string,bytes)'](EVM_CHAIN, token1.address)).to.eq(false)
    expect(await lpKeeper['isKnownToken(string,bytes)'](EVM_CHAIN, token2.address)).to.eq(false)
    expect(await lpKeeper['totalBalance(string,bytes)'](EVM_CHAIN, token1.address)).to.eq(0)
    expect(await lpKeeper['totalBalance(string,bytes)'](EVM_CHAIN, token2.address)).to.eq(0)
    ...
    expect(await lpKeeper['isKnownTokenUser(string,bytes,string,bytes)'](EVM_CHAIN, token1.address, EVM_CHAIN, other.address)).to.eq(false)
    ...
    expect(await lpKeeper['isKnownTokenUser(string,bytes,string,bytes)'](EVM_CHAIN, token2.address, EVM_CHAIN, other.address)).to.eq(false)
    expect(await lpKeeper['balance(string,bytes,string,bytes)'](EVM_CHAIN, token1.address, EVM_CHAIN, wallet.address)).to.eq(0)
    expect(await lpKeeper['balance(string,bytes,string,bytes)'](EVM_CHAIN, token1.address, EVM_CHAIN, other.address)).to.eq(0)
    expect(await lpKeeper['balance(string,bytes,string,bytes)'](EVM_CHAIN, token2.address, EVM_CHAIN, wallet.address)).to.eq(0)
    expect(await lpKeeper['balance(string,bytes,string,bytes)'](EVM_CHAIN, token2.address, EVM_CHAIN, other.address)).to.eq(0)
  ...
  describe('#isKnownToken(string,bytes)', () => {
    ...
    it('returns true for the known token', async () => {
      await openToken1()
      expect(await lpKeeper['isKnownToken(string,bytes)'](EVM_CHAIN, token1.address)).to.eq(true)
    })
  })
  ...
  describe('#tokenIdByChainAddress', () => {
    it('fails for the token that is not known', async () => {
      await expect(lpKeeper.tokenIdByChainAddress(EVM_CHAIN, wallet.address)).to.be.reverted
    })

    it('returns id for the known token', async () => {
      await openToken1()
      expect(await lpKeeper.tokenIdByChainAddress(EVM_CHAIN, token1.address)).to.eq(0)
    })
  })
  ...
  describe('#isKnownTokenUser(string,bytes,uint256)', () => {
    ...
    it('returns false when token user is not known', async () => {
      await wallet1Token1()
      expect(await lpKeeper['isKnownTokenUser(string,bytes,uint256)'](EVM_CHAIN, token1.address, 1)).to.eq(false)
    })

    it('returns true when token user is known', async () => {
      await wallet1Token1()
      expect(await lpKeeper['isKnownTokenUser(string,bytes,uint256)'](EVM_CHAIN, token1.address, 0)).to.eq(true)
    })
  })
  ...
  describe('#isKnownTokenUser(uint256,string,bytes)', () => {
    ...
    it('returns false when token user is not known', async () => {
      await wallet1Token1()
      expect(await lpKeeper['isKnownTokenUser(uint256,string,bytes)'](0, EVM_CHAIN, other.address)).to.eq(false)
    })

    it('returns true when token user is known', async () => {
      await wallet1Token1()
      expect(await lpKeeper['isKnownTokenUser(uint256,string,bytes)'](0, EVM_CHAIN, wallet.address)).to.eq(true)
    })
  })
  ...
  describe('#isKnownTokenUser(string,bytes,string,bytes)', () => {
    ...
    it('returns true when token user is known', async () => {
      await wallet1Token1()
      expect(await lpKeeper['isKnownTokenUser(string,bytes,string,bytes)'](EVM_CHAIN, token1.address, EVM_CHAIN, wallet.address)).to.eq(true)
    })
  })
  ...
  describe('#tokenUser(string,bytes,uint256)', () => {
    it('fails if token is not known', async () => {
      await expect(lpKeeper['tokenUser(string,bytes,uint256)'](EVM_CHAIN, token1.address, 0)).to.be.reverted
    })
    ...
    it('fails if userIndex is larger than the number of users', async () => {
      await wallet1Token1()
      await expect(lpKeeper['tokenUser(string,bytes,uint256)'](EVM_CHAIN, token1.address, 1)).to.be.reverted
    })

    it('returns user id', async () => {
      await wallet1Token1()
      expect(await lpKeeper['tokenUser(string,bytes,uint256)'](EVM_CHAIN, token1.address, 0)).to.eq(0)
    })
  })
  ...
  describe('#totalTokenUsers(string,bytes)', () => {
    it('returns 0 if token is not known', async () => {
      expect(await lpKeeper['totalTokenUsers(string,bytes)'](EVM_CHAIN, token1.address)).to.eq(0)
    })
    ...
    it('returns the number of token users', async () => {
      await wallet1Token1()
      expect(await lpKeeper['totalTokenUsers(string,bytes)'](EVM_CHAIN, token1.address)).to.eq(1)
    })

    it('returns the number of token users', async () => {
      await walletOther1Token1()
      expect(await lpKeeper['totalTokenUsers(string,bytes)'](EVM_CHAIN, token1.address)).to.eq(2)
    })
  })
  ...
  describe('#balance(uint256,string,bytes)', () => {
    it('returns 0 if token is not known', async () => {
      expect(await lpKeeper['balance(uint256,string,bytes)'](0, EVM_CHAIN, wallet.address)).to.eq(0)
    })
    ...
    it('returns user balance', async () => {
      await wallet1Token1()
      expect(await lpKeeper['balance(uint256,string,bytes)'](0, EVM_CHAIN, wallet.address)).to.eq(1)
    })
  })

  describe('#balance(string,bytes,uint256)', () => {
    it('returns 0 if token is not known', async () => {
      expect(await lpKeeper['balance(string,bytes,uint256)'](EVM_CHAIN, token1.address, 0)).to.eq(0)
    })
    ...
    it('returns user balance', async () => {
      await wallet1Token1()
      expect(await lpKeeper['balance(string,bytes,uint256)'](EVM_CHAIN, token1.address, 0)).to.eq(1)
    })
  })
  ...
  describe('#balance(string,bytes,string,bytes)', () => {
    ...
    it('returns user balance', async () => {
      await wallet1Token1()
      expect(await lpKeeper['balance(string,bytes,string,bytes)'](EVM_CHAIN, token1.address, EVM_CHAIN, wallet.address)).to.eq(1)
    })
  })
  ...
  describe('#totalBalance(string,bytes)', () => {
    ...
    it('returns token balance', async () => {
      await wallet1Token1()
      expect(await lpKeeper['totalBalance(string,bytes)'](EVM_CHAIN, token1.address)).to.eq(1)
    })

    it('returns token balance', async () => {
      await walletOther1Token1()
      expect(await lpKeeper['totalBalance(string,bytes)'](EVM_CHAIN, token1.address)).to.eq(2)
    })
  })
  ...
  describe('#add(string,bytes,uint256,uint256)', () => {
    it('fails if caller is not allowed to add', async () => {
      await expect(lpKeeper['add(string,bytes,uint256,uint256)'](EVM_CHAIN, token1.address, 1, 1)).to.be.reverted
    })
    ...
    it('records a new user', async () => {
      ...
      await openWallet()
      await lpKeeper.setCanAdd(wallet.address, true)
      await lpKeeper['add(string,bytes,uint256,uint256)'](EVM_CHAIN, token1.address, 0, 1)
      expect(await lpKeeper['isKnownTokenUser(string,bytes,uint256)'](EVM_CHAIN, token1.address, 0)).to.eq(true)
      expect(await lpKeeper['totalTokenUsers(string,bytes)'](EVM_CHAIN, token1.address)).to.eq(1)
    })
    ...
    it('adds each value to total balance', async () => {
      ...
      await lpKeeper.setCanAdd(wallet.address, true)
      await lpKeeper['add(string,bytes,uint256,uint256)'](EVM_CHAIN, token1.address, 0, 1)
      await lpKeeper['add(string,bytes,uint256,uint256)'](EVM_CHAIN, token1.address, 1, 1)
      expect(await lpKeeper['totalBalance(string,bytes)'](EVM_CHAIN, token1.address)).to.eq(2)
    })
    ...
    it('emits event', async () => {
      ...
      await openWallet()
      await lpKeeper.setCanAdd(wallet.address, true)
      await expect(lpKeeper['add(string,bytes,uint256,uint256)'](EVM_CHAIN, token1.address, 0, 1))
        .to.emit(lpKeeper, 'Add')
        .withArgs(wallet.address, 0, 0, 1)
    })
  })
  ...
  describe('#add(uint256,string,bytes,uint256)', () => {
    it('fails if caller is not allowed to add', async () => {
      await expect(lpKeeper['add(uint256,string,bytes,uint256)'](0, EVM_CHAIN, other.address, 1)).to.be.reverted
    })
    ...
    it('records a new user', async () => {
      ...
      await openWallet()
      await lpKeeper.setCanAdd(wallet.address, true)
      await lpKeeper['add(uint256,string,bytes,uint256)'](0, EVM_CHAIN, wallet.address, 1)
      expect(await lpKeeper['isKnownTokenUser(uint256,string,bytes)'](0, EVM_CHAIN, wallet.address)).to.eq(true)
      expect(await lpKeeper['totalTokenUsers(uint256)'](0)).to.eq(1)
    })
    ...
    it('adds each value to total balance', async () => {
      ...
      await lpKeeper.setCanAdd(wallet.address, true)
      await lpKeeper['add(uint256,string,bytes,uint256)'](0, EVM_CHAIN, wallet.address, 1)
      await lpKeeper['add(uint256,string,bytes,uint256)'](0, EVM_CHAIN, other.address, 1)
      expect(await lpKeeper['totalBalance(uint256)'](0)).to.eq(2)
    })
    ...
    it('emits event', async () => {
      ...
      await openWallet()
      await lpKeeper.setCanAdd(wallet.address, true)
      await expect(lpKeeper['add(uint256,string,bytes,uint256)'](0, EVM_CHAIN, wallet.address, 1))
        .to.emit(lpKeeper, 'Add')
        .withArgs(wallet.address, 0, 0, 1)
    })
  })
  ...
  describe('#add(string,bytes,string,bytes,uint256)', () => {
    it('fails if caller is not allowed to add', async () => {
      await expect(lpKeeper['add(string,bytes,string,bytes,uint256)'](EVM_CHAIN, token1.address, EVM_CHAIN, other.address, 1)).to.be.reverted
    })
    ...
    it('records a new user', async () => {
      ...
      await openWallet()
      await lpKeeper.setCanAdd(wallet.address, true)
      await lpKeeper['add(string,bytes,string,bytes,uint256)'](EVM_CHAIN, token1.address, EVM_CHAIN, wallet.address, 1)
      expect(await lpKeeper['isKnownTokenUser(string,bytes,string,bytes)'](EVM_CHAIN, token1.address, EVM_CHAIN, wallet.address)).to.eq(true)
      expect(await lpKeeper['totalTokenUsers(string,bytes)'](EVM_CHAIN, token1.address)).to.eq(1)
    })
    ...
    it('adds each value to total balance', async () => {
      ...
      await lpKeeper.setCanAdd(wallet.address, true)
      await lpKeeper['add(string,bytes,string,bytes,uint256)'](EVM_CHAIN, token1.address, EVM_CHAIN, wallet.address, 1)
      await lpKeeper['add(string,bytes,string,bytes,uint256)'](EVM_CHAIN, token1.address, EVM_CHAIN, other.address, 1)
      expect(await lpKeeper['totalBalance(string,bytes)'](EVM_CHAIN, token1.address)).to.eq(2)
    })
    ...
    it('emits event', async () => {
      ...
      await openWallet()
      await lpKeeper.setCanAdd(wallet.address, true)
      await expect(lpKeeper['add(string,bytes,string,bytes,uint256)'](EVM_CHAIN, token1.address, EVM_CHAIN, wallet.address, 1))
        .to.emit(lpKeeper, 'Add')
        .withArgs(wallet.address, 0, 0, 1)
    })
  })
  ...
  describe('#subtract(string,bytes,uint256,uint256)', () => {
    it('fails if caller is not allowed to subtract', async () => {
      await wallet2Token1()
      await expect(lpKeeper['subtract(string,bytes,uint256,uint256)'](EVM_CHAIN, token1.address, 0, 1)).to.be.reverted
    })
    ...
    it('subtracts from user balance', async () => {
      await wallet2Token1()
      await lpKeeper.setCanSubtract(wallet.address, true)
      await lpKeeper['subtract(string,bytes,uint256,uint256)'](EVM_CHAIN, token1.address, 0, 1)
      expect(await lpKeeper['balance(string,bytes,uint256)'](EVM_CHAIN, token1.address, 0)).to.eq(1)
    })
    ...
    it('emits event', async () => {
      await wallet2Token1()
      await lpKeeper.setCanSubtract(wallet.address, true)
      await expect(lpKeeper['subtract(string,bytes,uint256,uint256)'](EVM_CHAIN, token1.address, 0, 1))
        .to.emit(lpKeeper, 'Subtract')
        .withArgs(wallet.address, 0, 0, 1)
    })
  })
  ...
  describe('#subtract(uint256,string,bytes,uint256)', () => {
    it('fails if caller is not allowed to subtract', async () => {
      await wallet2Token1()
      await expect(lpKeeper['subtract(uint256,string,bytes,uint256)'](0, EVM_CHAIN, wallet.address, 1)).to.be.reverted
    })
    ...
    it('subtracts from user balance', async () => {
      await wallet2Token1()
      await lpKeeper.setCanSubtract(wallet.address, true)
      await lpKeeper['subtract(uint256,string,bytes,uint256)'](0, EVM_CHAIN, wallet.address, 1)
      expect(await lpKeeper['balance(uint256,string,bytes)'](0, EVM_CHAIN, wallet.address)).to.eq(1)
    })
    ...
    it('emits event', async () => {
      await wallet2Token1()
      await lpKeeper.setCanSubtract(wallet.address, true)
      await expect(lpKeeper['subtract(uint256,string,bytes,uint256)'](0, EVM_CHAIN, wallet.address, 1))
        .to.emit(lpKeeper, 'Subtract')
        .withArgs(wallet.address, 0, 0, 1)
    })
  })
  ...
  describe('#subtract(string,bytes,string,bytes,uint256)', () => {
    it('fails if caller is not allowed to subtract', async () => {
      await wallet2Token1()
      await expect(lpKeeper['subtract(string,bytes,string,bytes,uint256)'](EVM_CHAIN, token1.address, EVM_CHAIN, wallet.address, 1)).to.be.reverted
    })
    ...
    it('subtracts from user balance', async () => {
      await wallet2Token1()
      await lpKeeper.setCanSubtract(wallet.address, true)
      await lpKeeper['subtract(string,bytes,string,bytes,uint256)'](EVM_CHAIN, token1.address, EVM_CHAIN, wallet.address, 1)
      expect(await lpKeeper['balance(string,bytes,string,bytes)'](EVM_CHAIN, token1.address, EVM_CHAIN, wallet.address)).to.eq(1)
    })
    ...
    it('emits event', async () => {
      await wallet2Token1()
      await lpKeeper.setCanSubtract(wallet.address, true)
      await expect(lpKeeper['subtract(string,bytes,string,bytes,uint256)'](EVM_CHAIN, token1.address, EVM_CHAIN, wallet.address, 1))
        .to.emit(lpKeeper, 'Subtract')
        .withArgs(wallet.address, 0, 0, 1)
    })
  })
