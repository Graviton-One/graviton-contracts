...
import { claimGTONAbsoluteFixture, TEST_START_TIME } from './shared/fixtures'
...
describe.only('ClaimGTONAbsolute', () => {
  ...
  let voter: VoterV2
  let claimGTON: MockTimeClaimGTONAbsolute

  beforeEach('deploy test contracts', async () => {
    ;({ token0, token1, token2, balanceKeeper, voter, claimGTON } = await loadFixture(claimGTONAbsoluteFixture))
  ...
  describe('#claim', () => {
    ...
    it("does not transfer more than 100 at once if limit is activated", async function () {
      await claimGTON.setClaimActivated(true)
      await claimGTON.setLimitActivated(true)
      await balanceKeeper.setCanSubtract(claimGTON.address, true)
      await voter.setCanCheck(claimGTON.address, true)
    ...
    it("does not transfer more than 100 over the course of a day if limit is activated", async function () {
      await claimGTON.setClaimActivated(true)
      await claimGTON.setLimitActivated(true)
      await balanceKeeper.setCanSubtract(claimGTON.address, true)
      await voter.setCanCheck(claimGTON.address, true)
      await token0.approve(claimGTON.address, expandTo18Decimals(100))
      await claimGTON.claim(expandTo18Decimals(100))
      await expect(claimGTON.claim(expandTo18Decimals(1))).to.be.reverted
    })

    it("transfers less than 100 over the course of two days if limit is activated", async function () {
      await claimGTON.setClaimActivated(true)
      await claimGTON.setLimitActivated(true)
      await balanceKeeper.setCanSubtract(claimGTON.address, true)
      await voter.setCanCheck(claimGTON.address, true)
    ...
    it("updates limit variables if limit is activated", async function () {
      ...
      expect(await claimGTON.limitMax(wallet.address)).to.eq(expandTo18Decimals(55))
