...
import { FTM, BSC, PLG, ETH, AVA, DAI, HEC } from '../services/constants.ts'
 ...
 export default Vue.extend({
     ...
     methods: {
         ...
         async update () {
             this.clear()
             this.consul1_FTM = await this.invoker.balanceNT(FTM.provider, FTM.consul1)
             this.consul1_PLG = await this.invoker.balanceNT(PLG.provider, FTM.consul1)
             this.consul1_BSC = await this.invoker.balanceNT(BSC.provider, FTM.consul1)
             this.consul1_ETH = await this.invoker.balanceNT(ETH.provider, FTM.consul1)
             this.consul1_DAI = await this.invoker.balanceNT(DAI.provider, FTM.consul1)
             this.consul1_AVA = await this.invoker.balanceNT(AVA.provider, FTM.consul1)
             this.consul1_HEC = await this.invoker.balanceNT(HEC.provider, FTM.consul1)
             this.consul2_FTM = await this.invoker.balanceNT(FTM.provider, FTM.consul2)
             this.consul2_PLG = await this.invoker.balanceNT(PLG.provider, FTM.consul2)
             this.consul2_BSC = await this.invoker.balanceNT(BSC.provider, FTM.consul2)
             this.consul2_ETH = await this.invoker.balanceNT(ETH.provider, FTM.consul2)
             this.consul2_DAI = await this.invoker.balanceNT(DAI.provider, FTM.consul2)
             this.consul2_AVA = await this.invoker.balanceNT(AVA.provider, FTM.consul2)
             this.consul2_HEC = await this.invoker.balanceNT(HEC.provider, FTM.consul2)
             this.consul3_FTM = await this.invoker.balanceNT(FTM.provider, FTM.consul3)
             this.consul3_PLG = await this.invoker.balanceNT(PLG.provider, FTM.consul3)
             this.consul3_BSC = await this.invoker.balanceNT(BSC.provider, FTM.consul3)
             this.consul3_ETH = await this.invoker.balanceNT(ETH.provider, FTM.consul3)
             this.consul3_DAI = await this.invoker.balanceNT(DAI.provider, FTM.consul3)
             this.consul3_AVA = await this.invoker.balanceNT(AVA.provider, FTM.consul3)
             this.consul3_HEC = await this.invoker.balanceNT(HEC.provider, FTM.consul3)
             this.relay_FTM =   await this.invoker.balanceOf(FTM.provider, FTM.gton, FTM.relayLock)
             this.relay_PLG =   await this.invoker.balanceOf(PLG.provider, PLG.gton, PLG.relayLock)
             this.relay_BSC =   await this.invoker.balanceOf(BSC.provider, BSC.gton, BSC.relayLock)
             this.relay_ETH =   await this.invoker.balanceOf(ETH.provider, ETH.gton, ETH.relayLock)
             this.relay_DAI =   await this.invoker.balanceOf(DAI.provider, DAI.gton, DAI.relayLock)
             this.relay_AVA =   await this.invoker.balanceOf(AVA.provider, AVA.gton, AVA.relayLock)
             this.relay_HEC =   await this.invoker.balanceOf(HEC.provider, HEC.gton, HEC.relayLock)
             this.team_FTM = await this.invoker.balanceNT(FTM.provider, FTM.team)
             this.team_PLG = await this.invoker.balanceNT(PLG.provider, FTM.team)
             this.team_BSC = await this.invoker.balanceNT(BSC.provider, FTM.team)
             this.team_ETH = await this.invoker.balanceNT(ETH.provider, FTM.team)
             this.team_DAI = await this.invoker.balanceNT(DAI.provider, FTM.team)
             this.team_AVA = await this.invoker.balanceNT(AVA.provider, FTM.team)
             this.team_HEC = await this.invoker.balanceNT(HEC.provider, FTM.team)
             this.team_gton_FTM = await this.invoker.balanceOf(FTM.provider, FTM.gton, FTM.team)
             this.team_gton_PLG = await this.invoker.balanceOf(PLG.provider, PLG.gton, FTM.team)
             this.team_gton_BSC = await this.invoker.balanceOf(BSC.provider, BSC.gton, FTM.team)
             this.team_gton_ETH = await this.invoker.balanceOf(ETH.provider, ETH.gton, FTM.team)
             this.team_gton_DAI = await this.invoker.balanceOf(DAI.provider, DAI.gton, FTM.team)
             this.team_gton_AVA = await this.invoker.balanceOf(AVA.provider, AVA.gton, FTM.team)
             this.team_gton_HEC = await this.invoker.balanceOf(HEC.provider, HEC.gton, FTM.team)
