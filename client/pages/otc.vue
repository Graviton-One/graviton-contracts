  ...
  <div class="container">
    <div>
      <div>
        ...
        <div> Current network: {{ chain }}, OTC address: {{ otc }} </div>
        ...
        <div> Amount: <input v-model="exchangeAmount" placeholder="amount"></div>
        <Button class='button--green' size="large" ghost @click="exchange">Exchange</Button>
        <div> Address: <input v-model="address" placeholder="address"></div>
        <Button class='button--green' size="large" ghost @click="info">Info</Button>
        <table class="table table-hover">
            <thead>
                <tr>
                    <td>cliff</td>
            ...
            <tbody>
                <tr>
                    ...
                    <td>{{ formatUnits(vested, 18) }}</td>
                    <td>{{ formatUnits(claimed, 18) }}</td>
                    <td>{{ claimLast == 0 ? "" : formatTimestamp(claimLast) }}</td>
                    <td>{{ formatUnits(claimable, 18) }}</td>
                </tr>
            </tbody>
        </table>
      </div>
 ...
 export default Vue.extend({
     ...
     methods: {
         ...
         async info () {
             ...
             this.claimable = await this.invoker.claimable(this.otc, this.address)
             } catch(e) { console.log(e) }
         },
         async exchange () {
             await this.invoker.approve(this.token, this.otc, this.exchangeAmount)
