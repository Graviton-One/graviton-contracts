...
interface IOracleParserV2 {
    ...
    function deserializeAddress(bytes memory b, uint startPos) external pure returns (address);
    function bytesToBytes32(bytes memory b, uint offset) external pure returns (bytes32);
    function bytesToBytes16(bytes memory b, uint offset) external pure returns (bytes16);
