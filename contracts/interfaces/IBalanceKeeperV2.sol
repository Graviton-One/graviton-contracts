...
interface IBalanceKeeperV2 {
    ...
    function addByChainAddress(string memory chain, string memory addr, uint amount) external;
    function subtractById(uint id, uint amount) external;
    function subtractByChainAddress(string memory chain, string memory addr, uint amount) external;
